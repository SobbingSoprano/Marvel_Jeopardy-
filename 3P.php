...
if (isset($_POST['player']) && isset($_POST['guess'])) {
    ...
    if ($player === 1 && $_SESSION['player1_guess'] === null && $guess !== $_SESSION['player2_guess'] && $guess !== $_SESSION['player3_guess']) {
        $_SESSION['player1_guess'] = $guess;
    } elseif ($player === 2 && $_SESSION['player2_guess'] === null && $guess !== $_SESSION['player1_guess'] && $guess !== $_SESSION['player3_guess']) {
        $_SESSION['player2_guess'] = $guess;
    } elseif ($player === 3 && $_SESSION['player3_guess'] === null && $guess !== $_SESSION['player1_guess'] && $guess !== $_SESSION['player2_guess']) {
        $_SESSION['player3_guess'] = $guess;
    }
    // Check if all players have guessed
    if ($_SESSION['player1_guess'] !== null && $_SESSION['player2_guess'] !== null && $_SESSION['player3_guess'] !== null) {
        $target = $_SESSION['target_number'];
        $diffs = [
            1 => abs($target - $_SESSION['player1_guess']),
            2 => abs($target - $_SESSION['player2_guess']),
            3 => abs($target - $_SESSION['player3_guess'])
        ];
        $minDiff = min($diffs);
        $winners = array_keys($diffs, $minDiff);
        // If tie, pick randomly among tied players
        $_SESSION['current_turn'] = $winners[array_rand($winners)];
...
$player3Score = $_SESSION['player3_score'] ?? 0;
?>
<!DOCTYPE html>
<html lang="en">
...
<body class="game-page">
    ...
    <!-- Title -->
    <div class="title">
        <a href="index.html" class="title-wrapper" onclick="return confirm('Warning: this will reset the game and return you to the homepage. Continue?');">
    ...
    <?php if (!$gameStarted): ?>
        <!-- Number Guessing Screen for 3 Players -->
        <div class="question-overlay">
            <div class="question-card number-guess-card">
                ...
                <div class="guess-container">
                        ...
                        <?php if ($player2Guess === null): ?>
                            <form method="POST" action="3P.php" class="guess-form">
                                ...
                                <input type="number" id="guess2" name="guess" min="1" max="50" required
                                    <?php if ($player1Guess !== null || $player3Guess !== null) echo 'autofocus'; ?>>
                                <button type="submit" class="submit-btn">Submit</button>
                            </form>
                        <?php else: ?>
                            <div class="guess-submitted">✓ Number submitted!</div>
                    ...
                    <div class="player-guess-section">
                        <h3>Player 3</h3>
                        <?php if ($player3Guess === null): ?>
                            <form method="POST" action="3P.php" class="guess-form">
                                <input type="hidden" name="player" value="3">
                                <label for="guess3">Pick a number (1-50):</label>
                                <input type="number" id="guess3" name="guess" min="1" max="50" required
                                    <?php if ($player1Guess !== null || $player2Guess !== null) echo 'autofocus'; ?>>
                                <button type="submit" class="submit-btn">Submit</button>
                            </form>
                        <?php else: ?>
                            <div class="guess-submitted">✓ Number submitted!</div>
                ...
                <?php if ($player1Guess !== null && $player2Guess !== null && $player3Guess !== null): ?>
                    <div class="guess-results">
                        ...
                        <p>Player 3 guessed: <strong><?php echo $player3Guess; ?></strong> (off by <?php echo abs($targetNumber - $player3Guess); ?>)</p>
    ...
    <?php if ($showDailyDoubleWager && $gameStarted): ?>
        <!-- Daily Double Wager Screen for 3 Players -->
        <?php
        ...
        if ($currentTurn == 1) {
            ...
            $currentPlayerScore = $player3Score;
        }
    ...
    <div class="jeopardy-flex-row" style="flex-direction: row; align-items: flex-start; justify-content: center;">
        <!-- Left column: Player 1 and Player 3 stacked -->
        <div style="display: flex; flex-direction: column; gap: 2em; align-items: flex-end;">
            ...
            <div class="player-block <?php echo ($gameStarted && $currentTurn == 3) ? 'active-player' : ''; ?>">
                <div class="score-box">$<?php echo $player3Score; ?></div>
                <div class="player-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="64" height="64" fill="<?php echo ($gameStarted && $currentTurn == 3) ? '#ffffff' : '#ff0000'; ?>">
                        <circle cx="24" cy="18" r="12" />
                        <rect x="8" y="32" width="32" height="12" rx="6" />
                    </svg>
                </div>
                ...
                foreach ($values as $val) {
                    ...
                    foreach ($categories as $cat) {
                        ...
                        if ($isUsed) {
                            ...
                            $link = $isDailyDouble 
                                ? "3P.php?show_wager=1&category={$cat}&value={$val}"
                                : "3P.php?category={$cat}&value={$val}";
        ...
        <div style="display: flex; flex-direction: column; gap: 2em; align-items: flex-start;">
